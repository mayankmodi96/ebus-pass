...
if(isset($_POST['save']))
{
	...
	{
		if(file_exists('handicap/employee/userphoto/'.$_FILES['userfile']['name']))
		{
				echo $_FILES['userfile']['name']."already exist";
			if(file_exists('handicap/employee/adharphoto/'.$_FILES['adharfile']['name']))
			{
				echo $_FILES['adharfile']['name']."already exist";
			}
			if(file_exists('handicap/employee/disabilityphoto/'.$_FILES['dfile']['name']))
			{
				echo $_FILES['dfile']['name']."already exist";
			}
		}
		else if(move_uploaded_file($_FILES['userfile']['tmp_name'],
				'handicap/employee/userphoto/'.$_FILES['userfile']['name']))
		{
				$uphoto = $_FILES['userfile']['name'];

			if(move_uploaded_file($_FILES['adharfile']['tmp_name'],
				'handicap/employee/adharphoto/'.$_FILES['adharfile']['name']))
			{
				$aphoto = $_FILES['adharfile']['name'];
			}
			if(move_uploaded_file($_FILES['dfile']['tmp_name'],
				'handicap/employee/disabilityphoto/'.$_FILES['dfile']['name']))
			{
			...
			$insert = mysql_query("INSERT INTO emp_handi_pass(id,category,fname,mname,lname,gender,dob,address,pin,cno,eid,adharno,user_image,adhar_image,tod,pod,did,d_image,place_sit,change_place,alight_place,trano,changeno,jstart,jend,passmonth)
				VALUES('$usid','$category','$fname','$mname','$lname','$gender','$dob','$address','$pin','$cno','$eid','$adharno','$uphoto','$aphoto','$tod','$pod','$did','$dphoto','$place_sit','$change_place','$alight_place','$trano','$changeno','$jstart','$jend','$passmonth')");
...
<!-- start-smoth-scrolling -->
	...
	<body>
		...
		<div class="container hstyle">
			<h3>Apply pass for handicap employee</h3>
			<h5>* All details are mandatory field.</h5>
			<h5>* Image upload size less than(<) 500kb.</h5>
			<div class="col-md-12">
			<form method="post" name="applyforhandicapemployee" enctype="multipart/form-data">
				<div class="form-box">
					...
					<div class="row">
						...
						<div class="form-group col-md-4">
							...
							<input type="date" class="form-control" name="dob" requried="" pattern="[A-Za- z]+" title="Please enter valid date"></input>	
						</div>
						<div class="form-group col-md-4">
					...
					<div class="row">
						...
						<div class="form-group col-md-4">
							...
							<input type="time" class="form-control" name="jstart" requried="" title="Please enter vaild time"></input>	
						</div>
						<div class="form-group col-md-4">
							<label for="jend">Journey end time:</label>
							<input type="time" class="form-control" name="jend" requried="" title="Please enter vaild time"></input>	
						</div>
					</div>
					<h4>Pass Details</h4>
					<div class="row">
