...
if(isset($_POST['save']))
{
	...
	$select = mysql_query("SELECT uname FROM registration WHERE uname='$uname'");
	...
	else
	{
		if($pass == $cpass)
		{
			...
			$sql = mysql_query("INSERT INTO registration(fname,lname,uname,pass,cpass,emailid,contactno) 
			
						VALUES ('$fname','$lname','$uname','$pass','$cpass','$emailid','$contactno')");
			if($sql)
			{
				echo "<script>alert('Record save successfully');document.location='signin.php'</script>";
			}
			else
			{
				echo "<script>alert('Record not save, please try agian');document.location='signup.php'</script>";
				mysql_close();
			}	
		}
		else
					...
					<form name="register" method="post">
						<div class="contact-info">
	 					...
	 					<div class="form-group">
	    					...
	    					<input type="text" class="form-control" name="fname" required="" pattern="[A-Za- z]+" title="Only ALphabet Character is Valid">
	  					</div>
	  					<div class="form-group">
	    					<label for="lname">Last Name:</label>
	    					<input type="text" class="form-control" name="lname" required="" pattern="[A-Za- z]+" title="Only ALphabet Character is Valid">
	  					</div>
	  					<div class="form-group">
	    					<label for="uname">User Name:</label>
	    					<input type="text" class="form-control" name="uname" required="" pattern="[A-Za- z]+ [0-9]+" title="Only ALphabet Character is Valid">
	  					</div>
	  					<div class="form-group">
	    					<label for="pass">Password:</label>
	    					<input type="password" class="form-control" name="pass" required="compare">
	  					</div> 
	  					<div class="form-group">
							<label for="cpass">Conform Password:</label>
							<input type="password" class="form-control" name="cpass" required="compare">
						</div>
						<div class="form-group">
	    					<label for="emailid">email id:</label>
	    					<input type="email" class="form-control" name="emailid" required="" title="please velid email_id: like abc@abc.com">
	  					</div>
						<div class="form-group">
	  					...
	  					<div align="center">
	  						<button type="submit" class="btn btn-danger" value="Sign up" name="save">Signup</button>
						</div>
					</form>
				</div>
			</div>
